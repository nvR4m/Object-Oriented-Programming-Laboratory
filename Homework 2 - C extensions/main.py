...
1. WRITE A FUNCTION THAT INTERCHANGES TO VARIABLES. IT IS NOT POSSIBLE TO USE PARAMETERS AS REFERENCE IN PYTHON SO I MODIFIED THE CODE A LITTLE. (utils.py)
2. WRTIE AND USE A FUNCTION 'f(n, isRoundedTo100)'. IF 'isRoundedTo100 == True' THE FUNCTION RETURNS THE CLOSEST MULTIPLE BY 100, OTHERWISE IT RETURNS THE CLOSEST INTEGER TO 'n'. (utils.py)
3. WRITE A MAX FUNCTION THAT RETURNS THE STUDENT OBJECT WITH THE HIGHER GRADE. (utils.py, Student.py)
4. WRITE A FUNCTION THAT RETURNS CURRENT DATE/TIME. (utils.py)
5. WRITE A FUNCTION THAT RETURNS THE SIZE OF A PARAMETER. (utils.py)
7. WRITE A FUNCTION THAT COMPUTES THE ABOSLUTE VALUE OF A COMPLEX NUMBER AND AN INTEGER VALUE. (utils.py, NrComplex.py)
8. WRITE A FUNCTION THAT RETURNS THE MAXIMUM OF THREE NUMBERS. (utils.py)
9. WRITE A FUNCTION THAT RETURNS THE SUM OF TWO NUMBERS. (utils.py)
10. MODIFY 'interchange' FUCNTION (utils.py)
11. WRITE A FUNCTION THAT TAKES TWO NUMBERS 'a' AND 'b' AND AN OPERATOR 'c' AND COMPUTES THE RESULT (utils.py)
12. WRITE 3 CLASSES: 'Cat', 'Person', 'Car'. WRITE FUNCTION THAT RETURNS THE MINIM AGE. WRITE FUNCTION THAT RETURNS TURE IF OBJECTS OF THE SAME TYPE ARE THE SAME AGE OR FALSE IF DIFFERENT OBJECTS (Cat.py, Person.py, Car.py, utils.py)
